...
namespace ActorTests
{
    public class ActorControllerTests
    {
        ...
        public void Get_ReturnsAllActors()
        {
            ...
            _mockActorService.Setup(x => x.Get()).Returns(actors);

            // Act
            var result = _actorController.Get();
        ...
        [Fact]
        public void GetActorById_ReturnsNotFound_WhenActorDoesNotExist()
        {
            // Arrange
            int id = 1;
            _mockActorService.Setup(x => x.GetActorById(id)).Returns((Actor)null);

            // Act
            var result = _actorController.GetActorById(id);
        ...
        [Fact]
        public void GetActorById_ReturnsOk_WhenActorExists()
        {
            // Arrange
            int id = 1;
            var actor = new Actor { Id = id, FirstName = "Tom", LastName = "Hanks" };
            _mockActorService.Setup(x => x.GetActorById(id)).Returns(actor);
        ...
        [Fact]
        public void AttachActorsToFilm_ReturnsNotFound_WhenFilmDoesNotExist()
        {
            // Arrange
            int filmId = 1;
            var attachActorsDTO = new AttachActorsDTO { ActorIds = new List<int> { 1, 2 }, CaracterName = "Test" };
            _mockFilmService.Setup(x => x.GetFilmById(filmId)).Returns((Film)null);

            // Act
            var result = _actorController.AttachActorsToFilm(filmId, attachActorsDTO);
        ...
        [Fact]
        public void AttachActorsToFilm_ReturnsNotFound_WhenSomeActorsDoNotExist()
        {
            // Arrange
            int filmId = 1;
            var attachActorsDTO = new AttachActorsDTO { ActorIds = new List<int> { 1, 2 }, CaracterName = "Test" };
            var film = new Film { Id = filmId, Title = "The Shawshank Redemption" };
            _mockFilmService.Setup(x => x.GetFilmById(filmId)).Returns(film);
            _mockActorService.Setup(x => x.GetActorsByIds(attachActorsDTO.ActorIds)).Returns(new List<Actor> { new Actor { Id = 1, FirstName = "Tom", LastName = "Hanks" } });
        ...
        [Fact]
        public void AttachActorsToFilm_ReturnsOk_WhenAllActorsExist()
        {
            // Arrange
            int filmId = 1;
            var attachActorsDTO = new AttachActorsDTO { ActorIds = new List<int> { 1, 2 }, CaracterName = "Test" };
            ...
            var actors = new List<Actor>
        {
            ...
            _mockFilmService.Setup(x => x.GetFilmById(filmId)).Returns(film);
            _mockActorService.Setup(x => x.GetActorsByIds(attachActorsDTO.ActorIds)).Returns(actors);
            _mockFilmService.Setup(x => x.Update(filmId, It.IsAny<Film>())).Verifiable();
